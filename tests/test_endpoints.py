...
def test_endpoint_get_list(asset_name):
    with responses.RequestsMock() as mocked_requests:
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}{asset_name}/{config.version}?offset=0&limit=10",
...
def test_endpoint_counts(asset_name):
    with responses.RequestsMock() as mocked_requests:
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}counts/{asset_name}/{config.version}?detailed=false",
            body=b"2",
            status=200,
        )
        endpoint = getattr(aiod, asset_name)
...
def test_endpoint_get_asset(asset_name):
    with responses.RequestsMock() as mocked_requests:
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}{asset_name}/{config.version}/1",
...
def test_endpoint_get_list_from_platform(asset_name):
    ...
    with responses.RequestsMock() as mocked_requests:
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}platforms/{platform_name}/{asset_name}/{config.version}?offset=0&limit=10",
            body=b'[{"resource_1": "info"},{"resource_2": "info"}]',
            status=200,
        )
        endpoint = getattr(aiod, asset_name)
...
def test_endpoint_get_asset_from_platform(asset_name):
    ...
    with responses.RequestsMock() as mocked_requests:
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}platforms/{platform_name}/{asset_name}/{config.version}/{platform_identifier}",
            body=b'{"resource":"fake_details"}',
            status=200,
        )
        endpoint = getattr(aiod, asset_name)
...
def test_get_content(asset_name):
    with responses.RequestsMock() as mocked_requests:
        ...
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}{asset_name}/{config.version}/1/content",
            body=res_body,
            status=200,
        )
        endpoint = getattr(aiod, asset_name)
...
def test_get_content_with_distribution_idx(asset_name):
    with responses.RequestsMock() as mocked_requests:
        ...
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}{asset_name}/{config.version}/1/content/2",
            body=res_body,
            status=200,
        )
        endpoint = getattr(aiod, asset_name)
...
def test_search(asset_with_search):
    ...
    with responses.RequestsMock() as mocked_requests:
        mocked_requests.add(
            responses.GET,
            f"{config.api_server}search/{asset_with_search}/{config.version}{query}",
...
def test_endpoint_get_asset_async(asset_name):
    ...
    with aioresponses() as mocked_responses:
        ...
        mocked_responses.get(
            config.api_server + f"{asset_name}/" + config.version + "/" + "3",
...
def test_endpoint_get_list_async(asset_name):
    ...
    with aioresponses() as mocked_responses:
        mocked_responses.get(
            f"{config.api_server}{asset_name}/{config.version}?offset=0&limit=2",
            payload=[{"resource_1": "info"}, {"resource_2": "info"}],
            status=200,
        )
        mocked_responses.get(
            f"{config.api_server}{asset_name}/{config.version}?offset=2&limit=1",
