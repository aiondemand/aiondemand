...
def test_bookmark_list(valid_refresh_token):
    ...
    assert bookmarks[0].identifier.startswith("data"), (
        "Each element should be parsed to a Bookmark"
    )


@responses.activate
def test_bookmark_create_preserves_offset_in_utc(valid_refresh_token):
    responses.post(
        "http://not.set/not_set/bookmarks?resource_identifier=valid_identifier",
        json={
            "resource_identifier": "valid_identifier",
            "created_at": "2025-09-23T13:01:14+02:00",
        },
        match=[matchers.header_matcher({"Authorization": "Bearer valid_access"})],
    )
    bookmark = aiod.bookmarks.register("valid_identifier")
    assert bookmark.created.isoformat() == "2025-09-23T11:01:14+00:00"


@responses.activate
def test_bookmark_list_preserves_offset_in_utc(valid_refresh_token):
    responses.get(
        "http://not.set/not_set/bookmarks",
        json=[
            {
            ...
            {
                "resource_identifier": "mdl_AbFo3gwAWsQ3HxLG90RC2GWH",
                "created_at": "2025-09-22T14:10:09+05:30",
            },
        ],
        match=[matchers.header_matcher({"Authorization": "Bearer valid_access"})],
    )
    bookmarks = aiod.bookmarks.get_list()
    assert bookmarks[0].created.isoformat() == "2025-09-23T11:01:14+00:00"
    assert bookmarks[1].created.isoformat() == "2025-09-22T08:40:09+00:00"
